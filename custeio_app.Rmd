...
pickerInput("uo","Selecione o órgão", unique(base_app$uo), selected =unique(base_app$uo) ,
            options = list(`actions-box` = TRUE),
            width = 400,
            multiple = T)


pickerInput("tipo_custeio","Selecione o tipo de custeio", unique(base_app$tipo_custeio), selected =unique(base_app$tipo_custeio) , options = list(`actions-box` = TRUE),width = 400,multiple = T)


base_react <- reactive(base_app %>% filter(uo %in% input$uo, tipo_custeio %in% input$tipo_custeio))


base_percentual <- reactive(base_react() %>% group_by(uo, uo_cod) %>% summarise(atual = sum(atual), anterior= sum(anterior)) %>% mutate(percentual = (atual/anterior-1)*100) %>% mutate(nominal = atual-anterior))

base_custeio <- reactive(base_react() %>% group_by( tipo_custeio) %>% summarise(atual = sum(atual), anterior= sum(anterior)) %>% mutate(percentual = (atual/anterior-1)*100) %>% mutate(nominal = atual-anterior))
```
...
ggplot(base_percentual()   , aes(x = reorder (uo_cod,(-nominal)), y = round((nominal)/1000000,1)))+ coord_flip() + 
...
ggplot(base_percentual()    , aes(x = reorder (uo_cod,(-percentual)), y = percentual))+ coord_flip() + 
...
ggplot(base_custeio()   , aes(x = reorder (tipo_custeio,(-nominal)), y = round((nominal)/1000000,1)))+ coord_flip() + 
...
ggplot(base_custeio()    , aes(x = reorder (tipo_custeio,(-percentual)), y = percentual))+ coord_flip() + 
